...
This sitepackage additionally provides a new Content Blocks [Basic](https://docs.typo3.org/p/friendsoftypo3/content-blocks/main/en-us/YamlReference/Root/Index.html#confval-root-basics) `TYPO3/Headers` (note the plural spelling) for the output of the header**s** palette including the fields `date`, `header_link` and `subheader`, which are not included in the Standard Basic `TYPO3/Header`.
